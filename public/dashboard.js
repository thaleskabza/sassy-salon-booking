...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Initialize listeners and initial load
  initEventListeners();
  await loadData();
  ...
  function getDateRange() {
    ...
    switch (currentRange) {
      ...
      case 'custom':
        ...
        return { from: new Date(fromInput).toISOString(), to: new Date(toInput).toISOString() };
      default:
  ...
  function renderCharts(data) {
    ...
    new Chart(charts.daily, {
      type: 'line',
      data: { labels: Object.keys(data.daily), datasets: [{ label: 'Bookings', data: Object.values(data.daily), fill: true, tension: 0.3 }] },
      options: getChartOptions(true)
    });
  }
  ...
  function renderTables(data) {
    const mappings = [ ['Service','Bookings','bookings'], ['Service','Revenue','revenue'], ['Client','Visits','clients'], ['Date','Bookings','daily'] ];
    mappings.forEach(([h1,h2,key],i) => renderTable(tables[key], data[key], [h1, h2]));
  }
  ...
  function renderTable(container, data, headers) {
    ...
    headers.forEach(h => { const th = document.createElement('th'); th.textContent = h; headerRow.append(th); });
    thead.append(headerRow); table.append(thead);
    const tbody = document.createElement('tbody');
    Object.entries(data).forEach(([k,v]) => { const row = document.createElement('tr'); row.innerHTML = `<td>${k}</td><td>${v.toLocaleString()}</td>`; tbody.append(row); });
  ...
  function showLoadingState() {
    ...
    kpiCards.forEach(c => { c.classList.add('loading'); c.innerHTML = '<div class="skeleton-loader"></div>'; });
  }
