...
export async function ensureServicesSeeded(tenantId) {
  const existing = await redis.sMembers(tk(tenantId, 'services:all'));
  if (existing.length === 0) {
    await Promise.all(
      DEFAULT_SERVICES.map(async (service) => {
        await redis.hSet(tk(tenantId, `service:${service.id}`), service);
        await redis.sAdd(tk(tenantId, 'services:all'), tk(tenantId, `service:${service.id}`));
      })
    );
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    await ensureServicesSeeded(tenantId);
    const ids = await redis.sMembers(tk(tenantId, 'services:all'));
